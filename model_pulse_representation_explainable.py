...
def predict_reconstructed_signal(signal, sample_rate, peaks):
    ...
    for i in range(len(peaks) - 1):
        ...
        if pulse_length > 1:
            ...
            with torch.no_grad():
                reconstructed_pulse, physio_vector, wear_vector = model(pulse_tensor)
...
class DisentangledAutoencoder(nn.Module):
    def __init__(self, target_len, hidden_dim=50, physio_dim=20, wear_dim=20):
        super().__init__()
        self.physio_dim = physio_dim
        self.wear_dim = wear_dim
...
def train(model, train_dataset, val_dataloader, model_path, num_epochs, batch_size, device):
